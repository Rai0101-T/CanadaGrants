...
import { grants } from './shared/schema.ts';
...
async function addGrants() {
  try {
    ...
    if (existingGrants.length > 0) {
      ...
      // Insert new grants
      const insertPromises = newGrants.map(grant => 
        db.insert(grants).values({
          ...
          documents: grant.documents || null,
          faqQuestions: grant.faqQuestions || null,
          faqAnswers: grant.faqAnswers || null
        })
      );
      ...
      // Insert all grants
      const insertPromises = grantsData.map(grant => 
        db.insert(grants).values({
          ...grant,
          createdAt: new Date().toISOString(),
          pros: grant.pros || [],
